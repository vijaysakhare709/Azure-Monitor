...
resource "azurerm_monitor_metric_alert" "Network_threshold_alert" {
  ...
  resource_group_name = azurerm_resource_group.vijay.name
  scopes              = [azurerm_linux_virtual_machine.vijhellaymahcine-1.id]
  ...
  depends_on = [
    azurerm_linux_virtual_machine.vijhellaymahcine-1,
...
resource "azurerm_monitor_activity_log_alert" "virtual_machine_operation" {
  ...
  criteria {
    resource_id    = azurerm_linux_virtual_machine.vijhellaymahcine-1.id
